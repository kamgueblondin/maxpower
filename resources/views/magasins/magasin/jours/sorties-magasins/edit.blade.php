                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('days.magasins.magasins.sorties',$sortie->magasin_jour_id) }}" class="btn btn-sm btn-primary">{{ __('Retours à l\'accueil') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                     <form method="post" action="{{ route('magasins-sorties.update', $sortie->id) }}" autocomplete="off">
                            @csrf
                            @method('PATCH')
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('quantites') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="hidden" name="magasin" value="{{ $sortie->magasin }}" required >
